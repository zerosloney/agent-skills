...
|------|----------|------|
| `@page` | 路由定义 | 可支持多个路由 |
| `@inject` | 依赖注入 | 或使用 `[Inject]` 属性 |
| `@bind` | 双向绑定 | 组件使用 `@bind-Value` |
| `[Parameter]` | 组件输入 | 必要时使用 `[EditorRequired]` |
| `EventCallback` | 组件事件 | 类型安全的回调 |
| `RenderFragment` | 子内容 | 灵活布局 |
| `CascadingValue` | 共享状态 | 自动传递给子组件 |
| `AuthorizeView` | 条件认证 UI | 或使用 `@attribute [Authorize]` |
